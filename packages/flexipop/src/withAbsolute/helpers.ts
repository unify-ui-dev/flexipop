import { adjustYForTopVisibilityAbsolute, canAdjustYToFitTopAbsolute, canAdjustYToFitBottomAbsolute, adjustXForVisibilityAbsolute, canAdjustXToFitRightAbsolute, canBeAlignedOnMiddleXAbsolute, canBeAlignedOnMiddleYAbsolute } from "./alignement";
...
export const determinePositionAbsolute = (
    { placement,
        refWidth,
        refTop,
        refLeft,
    ...
    const adjustContentVisibilityY = () => adjustYForTopVisibilityAbsolute(
        () => canAdjustYToFitTopAbsolute(refTop, refHeight, popperHeight, windowHeight),
        () => canAdjustYToFitBottomAbsolute(refTop, refHeight, popperHeight, windowHeight), refTop, refHeight, popperHeight)

    const adjustContentVisibilityX = () => adjustXForVisibilityAbsolute(
        () => canAdjustXToFitRightAbsolute(refLeft, windowWidth, popperWidth, refWidth),
        refLeft, popperWidth, windowWidth, refWidth
    )

    const calculateMiddleX = () => (
        canBeAlignedOnMiddleXAbsolute(popperWidth, refWidth, refLeft, windowWidth) ?
            refWidth / 2 - popperWidth / 2 : adjustContentVisibilityX()
    )

    const calculateMiddleY = () => (
        canBeAlignedOnMiddleYAbsolute(popperHeight, refHeight, refTop, windowHeight) ?
